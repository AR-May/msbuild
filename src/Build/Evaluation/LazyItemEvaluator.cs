...
namespace Microsoft.Build.Evaluation
{
        ...
        where D : class, IItemDefinition<M>
    {
        ...
        private static IEnumerable<string> ExpandAndEnumerateMetadataValueAndCondition(ICollection<ProjectMetadataElement> metadata, Expander<P, I> expander)
        {
            // Expand properties here, because a property may have a value which is an item reference (ie "@(Bar)"), and
            // if so we need to add the right item reference.
            foreach (var metadatumElement in metadata)
            {
                ...
                var valueWithPropertiesExpanded = expander.ExpandIntoStringLeaveEscaped(
                    metadatumElement.Value,
                    expanderOptions,
                    metadatumElement.Location);

                var conditionWithPropertiesExpanded = expander.ExpandIntoStringLeaveEscaped(
                    metadatumElement.Condition,
                    expanderOptions,
                    metadatumElement.ConditionLocation);

                yield return valueWithPropertiesExpanded;
                yield return conditionWithPropertiesExpanded;
            }
        }
        ...
        private void ProcessMetadataElements(ProjectItemElement itemElement, OperationBuilderWithMetadata operationBuilder)
        {
            if (itemElement.HasMetadata)
            {
                ...
                var itemsAndMetadataFound = ExpressionShredder.GetReferencedItemNamesAndMetadata(ExpandAndEnumerateMetadataValueAndCondition(itemElement.Metadata, _expander));
