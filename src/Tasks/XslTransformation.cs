    ...
    public class XslTransformation : TaskExtension, IMultiThreadableTask
    {
        ...
        public override bool Execute()
        {
            ...
            try
            {
                ...
                AbsolutePath? absoluteXslCompiledDllPath = XslCompiledDllPath != null ? TaskEnvironment.GetAbsolutePath(XslCompiledDllPath.ItemSpec) : null;
                xsltinput = new XsltInput(absoluteXslInputPath, XslContent, absoluteXslCompiledDllPath, Log, PreserveWhitespace);
            }
            ...
            try
            {
                ...
                {
                    string absoluteOutputPath = TaskEnvironment.GetAbsolutePath(_outputPaths[i].ItemSpec);
                    using (XmlWriter xmlWriter = XmlWriter.Create(absoluteOutputPath, xslct.OutputSettings))
                    {
        ...
        internal class XmlInput
        {
            ...
            public XmlReader CreateReader(int itemPos)
            {
                ...
                {
                    AbsolutePath filePath = _filePaths[itemPos];
                    return XmlReader.Create(new StreamReader(filePath), new XmlReaderSettings { CloseInput = true }, filePath);
                }
                else // xmlModes.Xml
                {
        ...
        internal class XsltInput
        {
            ...
            /// <summary>
            /// Contains the absolute path to XSLT file or compiled XSLT dll when in file modes.
            /// </summary>
            private readonly AbsolutePath? _filePath;

            /// <summary>
            /// Contains the raw XSLT content when in Xslt mode.
            /// </summary>
            private readonly string _xsltContent;
            ...
            /// <param name="xsltCompiledDllPath">The absolute path to compiled XSLT file or null.</param>
            /// <param name="logTool">Log helper.</param>
            /// <param name="preserveWhitespace">Flag for xslt whitespace option.</param>
            public XsltInput(AbsolutePath? xsltFilePath, string xslt, AbsolutePath? xsltCompiledDllPath, TaskLoggingHelper logTool, bool preserveWhitespace)
            {
                _log = logTool;
                if ((xsltFilePath != null && xslt != null) ||
                    (xsltFilePath != null && xsltCompiledDllPath != null) ||
                    (xslt != null && xsltCompiledDllPath != null))
                {
                    throw new ArgumentException(ResourceUtilities.GetResourceString("XslTransform.XsltInput.TooMany"));
                }
                else if (xsltFilePath == null && xslt == null && xsltCompiledDllPath == null)
                {
            ...
            public XslCompiledTransform LoadXslt(bool useTrustedSettings)
            {
                ...
                switch (_xslMode)
                {
                    case XslModes.Xslt:
                        {
                            using var sr = new StringReader(_xsltContent);
                    ...
                    case XslModes.XsltFile:
                        ...
                        {
                            _log.LogMessageFromResources(MessageImportance.Low, "XslTransform.UseTrustedSettings", _filePath.Value.Value);
                        }
